...
module.exports = {
  ...
  walletTrans: (data, callback) => {
    ...
    db.query(sqlq, [amount, receive, nick], (error, results, fields) => {
      if (error) {
        return callback(error);
      }
      //==== IN vào lịch sử
      ...
      db.query(
        `insert into exchange_history (email, nick_name, from_e, to_e, send, receive, status, created_at)
                values(?,?,?,?,?,?,?,now())`,
        [email, nick, type_s, type_r, amount, receive, 1],
      );
