  ...
  getAllChampions: (callback) => {
    db.query(
      ...
      async (error, results, fields) => {
        ...
        const getTopUserByUser = await Promise.all(
          results.map(async (e) => {
            ...
            if (e.type === "klgd") {
              return new Promise((resolve, reject) => {
                db.query(
                  `select email, sum(amount_win) as trade from bet_history where type_account = 1 and created_at BETWEEN '${dateStartCover}' and '${dateEndCover}' GROUP BY email ORDER BY trade DESC LIMIT 3`,
                  [],
                  (error1, results1) => {
                    if (error1) {
                      reject(error1);
            ...
            if (e.type === "hh") {
              const hoaHongCommissionHistory = await new Promise(
                (resolve, reject) => {
                  db.query(
                    `SELECT upline_id, SUM(personal_trading_volume) as trading_volumn
              ...
              if (!hoaHongCommissionHistory.length)
              ...
              const userInfo = await new Promise((resolve, reject) => {
                db.query(
                  `select email, ref_code from users where ref_code in ${refCodeIn}`,
                  [],
                  (error1, results2) => {
                    if (error1) {
                      reject(error1);
