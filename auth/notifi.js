...
async function SEND_COMMISSION_USER() {
  // tổng kết hoa hồng mỗi ngày
...
function SAVE_LOG_NOTIFI(type, cue, email, title, content) {
  db.query(
    `INSERT INTO notifi (cu_email, email, content, title, type, created_at)
    VALUES (?,?,?,?,?,now())`,
    [cue, email, content, title, type],
    (error, results, fields) => {
      console.log(error);
    },
  );
