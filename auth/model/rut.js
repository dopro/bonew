...
module.exports = async function (bot, id, text) {
  ...
  if (!!tentk) {
    bot.sendMessage(
      id,
      `=======🔔ĐANG THỰC HIỆN RÚT CHO MÃ <i>${tentk}</i>🔔========`,
    ...
    let kq = await new Promise((res, rej) => {
      db.query(
        ...
        (error, results, fields) => {
          if (error) {
            bot.sendMessage(id, `Lỗi truy vấn dữ liệu`, {
              parse_mode: "HTML",
    ...
    if (kq != null) {
      if (kq.length > 0) {
        ...
        axios
          .post(config.DOMAIN + "/api/trades/doneWithdrawalTele", obj)
          .then((res) => {
            let d = res.data;
            if (d.success == 1) {
              bot.sendMessage(
