  ...
  paypal.payment.execute(
    ...
    function (error, payment) {
      if (error) {
        ...
        let FEE =
          payment.transactions[0].related_resources[0].sale.transaction_fee
            .value; // phí thuế chiếc khấu
        ...
        db.query(
          ...
          (error, results, fields) => {
            ...
            if (!results[0]) {
              ...
              db.query(
                `INSERT INTO trade_history (pay_fee, paypal_order_id, from_u, to_u, paypal, type_key, type, currency, amount, note, status, created_at)
                        values(?,?,?,?,?,?,?,?,?,?,?,now())`,
                [
                  FEE,
                  paymentId,
