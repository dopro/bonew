  ...
  db.query(
    ...
    (err, result) => {
      ...
      if (Array.isArray(result) && result.length) {
         ...
         */
        db.query(
          `INSERT INTO trade_history (email, from_u, type_key, type, currency, amount, real_amount, status, bank, created_at)
      values(?,?,?,?,?,?,?,?,?,now())`,
          [email, n, "nt", "Nạp tiền (VNĐ)", "vnd", a, al, 0, b],
          (err, result1) => {
            if (err) {
              throw new Error(err);
            }
...
app.post("/approval", function (req, res) {
  const body = req.body;
  const { status, id, note, amount, email } = body;

  db.query(
    `UPDATE trade_history SET status = ?, note = ? WHERE id = ?`,
    [status, note, id],
    (err) => {
      if (err) {
        throw new Error(err);
